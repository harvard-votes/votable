...
app.get("/contacts", async (req, res) => {
  try {
    ...
    const contacts = query
      ...
      : await pool.query(
          "SELECT * FROM contacts ORDER BY last_name ASC, first_name ASC;"
        );
...
app.post("/team/add", async (req, res) => {
  try {
    ...
    const newContact = await pool.query(
      "INSERT INTO team (first_name,last_name,email,phone_number,hy,state,role,organizing_lead,notes,house,year) \
...
app.get("/team", async (req, res) => {
  try {
    ...
    const contacts = await pool.query(
      "SELECT * FROM team ORDER BY last_name ASC, first_name ASC;"
    );
...
app.get("/scripts", async (req, res) => {
  try {
    const allScripts = await pool.query(
      "SELECT * FROM scripts ORDER BY last_modified DESC"
    );
...
app.get("/scripts/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const script = await pool.query("SELECT * FROM scripts WHERE id = $1", [
      id,
    ]);
...
app.delete("/scripts/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const deleteScript = await pool.query("DELETE FROM scripts WHERE id = $1", [
      id,
    ]);
