...
function Table() {
  ...
  const handleCheckmarkChange = (e) => {
    ...
    //Get the index of key of contact. if -1 then not currently checked
    const checkedIndex = checked.indexOf(parseInt(e.target.name))
    if (checkedIndex == -1) {
      checkedArray.push(parseInt(e.target.name));
    } else {
      checkedArray.splice(checkedIndex, 1);
    }
    setChecked(checkedArray); //update
    // console.log(checked);
  }
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {contacts.map((contact) => (
                      <Contact key={contact.id} handleCheckmarkChange={handleCheckmarkChange} contact={contact} statuses={statuses} checked={checked.indexOf(parseInt(contact.id)) != -1 ? true : false} onDelete={deleteContact} />
                    ))}
                  </tbody>
                </table>
              </div>
